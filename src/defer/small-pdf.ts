...
async function uploadSmallPDF({ fileNameInGCP, originalFileName, jobID}: Props){
    try{
        const promise = new Promise(async (resolve, reject) => {

            await redisClient.set(jobID, JSON.stringify({status: 'processing'}), 'EX', 120)
            
            const text = await OCRFileContent('gs://pdf-source-storage-bucket/' + fileNameInGCP, fileNameInGCP, randomUUID(), 'application/pdf')
            const parsedText: string = JSON.parse(text as string).responses[0].fullTextAnnotation?.text!
