...
export async function uploadSplitFile(file: Buffer, fileName: string) {
  try {
    ...
    if (upload === 'done') {
      const getFileContent = await OCRFileContent(
        'gs://pdf-source-storage-bucket/' + fileName,
        fileName,
        randomID,
        'application/pdf',
      )
...
export default async function uploadFile(
  ...
  fileName?: string,
  sample?: boolean,
  bucket: string = 'pdf-source-storage-bucket',
) {
  try {
    const randomID = randomUUID()
      ...
      return await new Promise((resolve, reject) => {
        createReadStream((file as File).filepath)
          .pipe(
            storage
              .bucket(bucket)
              .file((file as File).newFilename as string)
              .createWriteStream({
                metadata: {
    ...
    if (!sample) {
      const upload =
        file instanceof Buffer ? await uploadBuffer() : await uploadFile()

      if (upload === 'done') {
        const getFileContent = await OCRFileContent(
          'gs://pdf-source-storage-bucket/' +
            ((file as File).newFilename || randomID),
          (file as File).newFilename || (fileName as string),
          randomID,
          ((file as File).mimetype as string) || 'application/pdf',
        )
