...
export default async function splitPDF(
  ...
  { fileName: string; fullDocumentText: string; randomID: string } | undefined
> {
  ...
  try {
    ...
    for (let sectionIndex = 0; sectionIndex < numSections; sectionIndex++) {
      ...
      sectionPromises.push({
        buffer: Buffer.from(sectionPDFBytes),
    ...
    const runConcurrently = async () => {
      const promise = await Promise.all(
        sectionPromises.map(
          async (obj: { buffer: Buffer; sectionName: string }) => {
            await createFileGCPStorage(
              'pdf-source-storage-bucket',
              obj.sectionName,
              obj.buffer,
            )
            const fileContent = await OCRFileContent(
              `gs://pdf-source-storage-bucket/${obj.sectionName}`,
              obj.sectionName,
              randomUUID(),
              'application/pdf',
            )
